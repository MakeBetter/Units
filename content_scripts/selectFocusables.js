...
// If a unit is selected, this is applies only within it, else on the whole page
function selectFocusable(nextOrPrev, beginningChar) {

    var $selectedUnit = $unitsArray[selectedUnitIndex];
    var $scope =  $selectedUnit || $document;
    ...
    if ($matchedFocusables && $matchedFocusables.length) {
        ...
        //if scope is the currently selected unit and the active element lies within it
        if ( ($scope === $selectedUnit && getContainingUnitIndex(activeEl) === selectedUnitIndex)
            || ($scope === $document)) {
